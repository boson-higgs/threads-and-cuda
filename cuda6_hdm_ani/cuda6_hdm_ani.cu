...
void Animation::start( CudaPic t_bg_pic, CudaPic t_ins_pic )
{
	...
	l_cerr = cudaMalloc( &m_cuda_ins_pic.m_p_void, m_cuda_ins_pic.m_size.x * m_cuda_ins_pic.m_size.y * sizeof( uchar4 ) );
	if ( l_cerr != cudaSuccess )
		printf( "CUDA Error [%d] - '%s'\n", __LINE__, cudaGetErrorString( l_cerr ) );

	// Memory for result
	l_cerr = cudaMalloc( &m_cuda_res_pic.m_p_void, m_cuda_res_pic.m_size.x * m_cuda_res_pic.m_size.y * sizeof( uchar3 ) );
	if ( l_cerr != cudaSuccess )
		printf( "CUDA Error [%d] - '%s'\n", __LINE__, cudaGetErrorString( l_cerr ) );
	...
	l_cerr = cudaMemcpy( m_cuda_ins_pic.m_p_void, t_ins_pic.m_p_void, m_cuda_ins_pic.m_size.x * m_cuda_ins_pic.m_size.y * sizeof( uchar4 ), cudaMemcpyHostToDevice );
	if ( l_cerr != cudaSuccess )
		printf( "CUDA Error [%d] - '%s'\n", __LINE__, cudaGetErrorString( l_cerr ) );
